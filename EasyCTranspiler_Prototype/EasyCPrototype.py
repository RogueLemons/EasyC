...
def add_check_nullchecks_locals(code):
    ...
    for line in lines:
        ...
        output.append(f"{indent}ALC__NULL__CHECK({var_name});")
...
def add_check_nullchecks_params(code):
    ...
    while i < len(lines):
        ...
        for var in vars_to_check:
            output.append(f"{first_line_indent}ALC__NULL__CHECK({var});")
...
def insert_nullcheck_macro_after_line(line_number, code):
    """
    Inserts the ALC__NULL__CHECK macro block immediately after the specified line number.
...
def has_nullcheck_macro(code):
    """
    Returns True if the C code contains a definition of ALC__NULL__CHECK macro,
    otherwise False.
    """
    # \s* allows optional spaces between #define and macro name
    pattern = re.compile(r'^\s*#define\s+ALC__NULL__CHECK\b', re.MULTILINE)
...
# -----------------------------
# Convert AlloyC headers and files to C in include statements
# -----------------------------
def normalize_includes(code: str) -> str:
    ...
    def replace(match):
        ...
        # Replace extensions
        if path.endswith(".alh"):
            path = path[:-3] + ".h"
        elif path.endswith(".alc"):
            path = path[:-3] + ".c"
...
# Transpile
# -----------------------------
def transpile_alc(code):

    # Process AlloyC keywords
...
if __name__ == "__main__":
    ...
    print(f"Done! AlloyC transpiled: {outfile}")